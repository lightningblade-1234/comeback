...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onToggle, userType }) => {
  ...
  const studentItems = [
    { title: 'Personal Care', url: '/talk', icon: Heart },
    { title: 'Resources & Self-Care', url: '/more', icon: BookOpen },
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={cn(
        "relative left-0 top-0 h-full bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border-r border-white/20 shadow-2xl z-50 transition-all duration-300 ease-in-out",
        // Keep a stable icon rail when closed. The old zero-width mobile
        // variant left padded controls protruding into the content column.
        isOpen ? "w-64" : "w-16",
        "translate-x-0"
      )}>
