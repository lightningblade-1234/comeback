...
function NextFeature({title,description}:{title:string;description:string}){return <><div className="eyebrow">Frontend foundation</div><h1>{title}</h1><Panel><p>{description}</p><p className="muted">This feature is assigned to the victim frontend developer. It is not connected yet.</p><Button asChild><Link to="/home">Back to Home</Link></Button></Panel></>;}
function VictimWorkspace(){
 const {t}=useTranslation();
 return <><a className="skip" href="#main">Skip to content</a><div className="app-shell"><aside className="sidebar"><div className="brand">haven<span style={{color:'var(--accent)'}}>.</span></div><p className="muted">A space for you</p><nav aria-label="Main navigation">{[['/home','home'],['/talk','talk'],['/support','support'],['/journal','journal'],['/more','more']].map(([path,label])=><NavLink end={path==='/home'} key={path} to={path}>{t(label)}</NavLink>)}</nav></aside><main id="main" className="main"><div className="demo-banner">Development demo · Synthetic records · No live counseling or emergency connection</div><Routes><Route path="/home" element={<Home/>}/><Route path="/support" element={<Home/>}/><Route path="/talk" element={<NextFeature title="Talk to Haven" description="Text, live voice and the fixed Haven companion will share one conversation here."/>}/><Route path="/journal" element={<NextFeature title="My Journal" description="Private reflections, draft saving and explicit counselor-sharing controls will live here."/>}/><Route path="/more" element={<NextFeature title="More support" description="Resources, My Case, Shared Support and preferences are planned for this area."/>}/><Route path="*" element={<NextFeature title="Page not found" description="This address does not match a page in the foundation."/>}/></Routes></main></div></>;
}
export function App(){
 return <Routes><Route path="/" element={<LandingPage/>}/><Route path="/*" element={<VictimWorkspace/>}/></Routes>;
}
