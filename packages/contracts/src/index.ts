...
export const AssistanceCategory=z.enum(['PROTECTION','COURT_TRANSPORT','SAFE_ACCOMMODATION','RELOCATION_REVIEW','FINANCIAL_ASSISTANCE','LEGAL_AID','MEDICAL_ASSESSMENT','REHABILITATION']);
export const DistrictAssistanceRequest=z.object({id:z.string(),docket:z.string(),title:z.string(),category:AssistanceCategory,priority:AuthorityPriority,status:InterventionState,description:z.string(),serviceAvailable:z.boolean().nullable(),dueAt:z.string().datetime(),lastHumanNote:z.string().nullable()});
export type DistrictAssistanceRequest=z.infer<typeof DistrictAssistanceRequest>;
export const AssistanceUpdatePayload=z.object({state:InterventionState,note:z.string().min(3,'Human note required')});
export type AssistanceUpdatePayload=z.infer<typeof AssistanceUpdatePayload>;
